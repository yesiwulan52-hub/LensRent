        ...
        <form action="{{ route('sewa.store') }}" method="POST">
                ...
                <select name="kamera_id" required>
                    ...
                    @foreach($kameras as $k)
                        <option value="{{ $k->id }}" {{ old('kamera_id') == $k->id ? 'selected' : '' }}>
                            {{ $k->nama }} ({{ $k->kode }}) - Stok: {{ $k->jumlah }} - Rp {{ number_format($k->harga, 0, ',', '.') }}/hari
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    @error('tanggal_sewa') <span class="error-msg">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    <label>Tanggal Kembali *</label>
                    <input type="date" name="tanggal_kembali" value="{{ old('tanggal_kembali') }}" required>
                    @error('tanggal_kembali') <span class="error-msg">{{ $message }}</span> @enderror
                </div>
            </div>
